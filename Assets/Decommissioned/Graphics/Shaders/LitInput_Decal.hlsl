...
UNITY_DOTS_INSTANCING_START(MaterialPropertyMetadata)
    ...
    UNITY_DOTS_INSTANCED_PROP(float4, _DetailAlbedoMapColor)
...
#define _DetailAlbedoMapColor   UNITY_ACCESS_DOTS_INSTANCED_PROP_FROM_MACRO(float4  , Metadata_DetailAlbedoMapColor)
...
half3 ApplyDetailAlbedo(float2 detailUv, half3 albedo, half detailMask)
{
    ...
    // Multiplied detailMask by 50 to make the detail map much clearer for text decals - B.S.
    return albedo * LerpWhiteTo(detailAlbedo, detailMask * 50);
#else
    return albedo;
#endif
}
