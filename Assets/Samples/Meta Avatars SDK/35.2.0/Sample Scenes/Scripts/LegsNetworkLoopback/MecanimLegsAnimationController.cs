...
namespace Oculus.Avatar2
{
    public class MecanimLegsAnimationController : MonoBehaviour, IUIControllerInterface
    {
        ...
        public Quaternion LowerBodyRotation
        {
            get => _lowerBodyRotation;
            set => _lowerBodyRotation = value;
        }
        ...
        private bool _shouldUpdateRootAngle = true;

        public bool ShouldUpdateRootAngle
        {
            get => _shouldUpdateRootAngle;
        }
        ...
        private bool GetHeadsetTrackingState(out OvrAvatarInputTrackingState state)
        {
            state = default;
            return _entity != null
                && _entity.InputManager != null
                && _entity.InputManager.InputTrackingProvider != null
                && _entity.InputManager.InputTrackingProvider.GetInputTrackingState(out state)
                && state.headsetActive;
        }
        ...
        protected Vector3 GetHeadVector()
        {
            if (_entity == null)
            {
                OvrAvatarLog.LogError("Unable to get head vector, no entity found");
                return Vector3.forward;
            }

            if (!GetHeadsetTrackingState(out var headsetState))
            {
                OvrAvatarLog.LogError("unable to get head vector, failed to get headset input");
                return Vector3.forward;
            }
            else
            {
                var headVector = _entity!.transform.InverseTransformDirection(headsetState.headset.orientation * Vector3.forward);
        ...
        {
            // fallback calculate to use headset position
            if (!GetHeadsetTrackingState(out var headsetState))
            {
                return null;
            }

            return headsetState.headset.position;
        }
        ...
        protected void StartSitting()
        {
            ...
            _startSittingDirection = _lowerBodyRotation * Vector3.forward;
