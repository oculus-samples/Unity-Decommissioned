...
namespace Unity.Netcode
{
    ...
    public sealed class NetworkObject : MonoBehaviour
    {
...
#if UNITY_EDITOR
        private static readonly uint DefaultGlobalObjectIdHash = XXHash.Hash32(default(UnityEditor.GlobalObjectId).ToString());
        ...
        public void GenerateGlobalObjectIdHash()
        {
            ...
            var hash = XXHash.Hash32(globalObjectIdString);
            if (hash == DefaultGlobalObjectIdHash)
            {
                return;
            }

            GlobalObjectIdHash = hash;
            // Debug.Log((name, old, GlobalObjectIdHash, new { gameObject.scene.name, gameObject.scene.path, gameObject.scene.handle, gameObject.scene.isLoaded, gameObject.scene.isSubScene }), this);
        }
#endif // UNITY_EDITOR
